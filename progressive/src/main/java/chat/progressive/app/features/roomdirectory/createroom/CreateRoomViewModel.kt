...
class CreateRoomViewModel @AssistedInject constructor(
    ...
    private fun initTodoSettings() {
        if (progressivePreferences.isTodoRoomsEnabled()) {
            setState {
                copy(
                        todoButtonPlacement = progressivePreferences.getTodoButtonPlacement()
                )
            }
        }
    }
    ...
    private fun doCreateRoom() = withState { state ->
        ...
        val createRoomParams = CreateRoomParams()
                .apply {
                    ...
                    val todoKeyword = if (state.isTodoRoom && progressivePreferences.isTodoRoomsEnabled()) {
                        progressivePreferences.getTodoKeyword()
                    } else null
                    topic = when {
