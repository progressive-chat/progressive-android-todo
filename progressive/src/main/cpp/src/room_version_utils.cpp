...
std::string getLatestStableVersion(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getLatestStableVersion" << R"(","ok":true)";
...
std::string versionSupports(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"versionSupports"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "versionSupports" << R"(","ok":true)";
...
std::string supportsKnockJoinRule(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"supportsKnockJoinRule"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "supportsKnockJoinRule" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string supportsRestrictedJoinRule(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"supportsRestrictedJoinRule"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "supportsRestrictedJoinRule" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string supportsEventRedaction(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"supportsEventRedaction"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "supportsEventRedaction" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string getRecommendedVersion(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getRecommendedVersion" << R"(","ok":true)";
...
std::string shouldUpgradeRoom(const std::string& json) {
    std::string input = parseJsonStringValue(json, "input");
    if (input.empty()) input = json;
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "shouldUpgradeRoom" << R"(","result":)" << (result ? "true" : "false");
