...
std::string buildRoomSettingsUpdate(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildRoomSettingsUpdate"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildRoomSettingsUpdate" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string formatRoomSettings(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatRoomSettings"})";
...
std::string getJoinRuleLabel(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getJoinRuleLabel" << R"(","ok":true)";
...
std::string getHistoryVisibilityLabel(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getHistoryVisibilityLabel" << R"(","ok":true)";
...
std::string getGuestAccessLabel(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getGuestAccessLabel" << R"(","ok":true)";
...
std::string isValidJoinRule(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isValidJoinRule" << R"(","result":)" << (result ? "true" : "false");
...
std::string isValidHistoryVisibility(const std::string& json) {
    std::string input = parseJsonStringValue(json, "input");
    if (input.empty()) input = json;
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isValidHistoryVisibility" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
